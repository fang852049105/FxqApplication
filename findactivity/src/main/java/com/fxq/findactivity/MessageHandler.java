...
public class MessageHandler extends Thread{
    ...
    public void run() {
        ...
        mHandler = new Handler(){
            ...
            public void handleMessage(Message msg) {
                switch (msg.what){
                    case ActivityMonitor.StackEvent.PUT:
                        MessageHandler.this.pushIntoStack(msg.arg1, (ActivityInstanceInfo) msg.obj);
                        break;
                    case ActivityMonitor.StackEvent.REMOVE:
    ...
    private void notification(ActivityMonitor.StackEvent stackEvent, ActivityInstanceInfo activityInstanceInfo){
    ...
    private void pushIntoStack(int flags,ActivityInstanceInfo activityInstanceInfo){
        ...
        for (TaskStack taskStack : mTaskStacks) {
            if (taskStack.getId() == taskId){
                ...
                notification(new ActivityMonitor.StackEvent(ActivityMonitor.StackEvent.PUT,taskStack),activityInstanceInfo);
                return;
            }
        }
    ...
    private void popFromStack(ActivityInstanceInfo activityInstanceInfo){
        ...
        notification(new ActivityMonitor.StackEvent(ActivityMonitor.StackEvent.REMOVE,removeFrom), activityInstanceInfo2);
    }
    ...
    private void notifyActivityChanges(ActivityInstanceInfo activityInstanceInfo){
        ...
        notification(new ActivityMonitor.StackEvent(ActivityMonitor.StackEvent.NONE,taskStack),instanceInfo);
    }
    ...
    private boolean clearTop(TaskStack taskStack, Class cls){
        Stack<ActivityInstanceInfo> activityInstanceInfoStack = taskStack.getActivityInstanceInfoStack();
        if (activityInstanceInfoStack == null || activityInstanceInfoStack.empty()) {
            return false;
        }
        int count = 0;
