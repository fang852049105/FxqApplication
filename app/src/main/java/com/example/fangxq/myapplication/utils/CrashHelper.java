...
//    private final static String TAG = AppConfig.DEBUG_TAG + "-" + CrashHelper.class.getSimpleName();
    ...
    public static void install(Context context, Class<? extends Activity> clazz) {
        try {
            if (context == null) {
                LogUtil.e(LogUtil.TAG, "Install failed: context is null!");
                return;
            }
            application = (Application) context.getApplicationContext();
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                LogUtil.w(LogUtil.TAG, "Crash tool will be installed, but may not be reliable in API lower than 14");
            }
            ...
            LogUtil.d(LogUtil.TAG, "current application package name is " + pkgName);
            if (oldHandler != null && oldHandler.getClass().getName().startsWith(pkgName)) {
                LogUtil.e(LogUtil.TAG, "You have already installed crash tool, doing nothing!");
                return;
            }
            if (oldHandler != null && !oldHandler.getClass().getName().startsWith("com.android.internal.os")) {
                LogUtil.e(LogUtil.TAG, "IMPORTANT WARNING! You already have an UncaughtExceptionHandler, are you sure this is correct? If you use ACRA, Crashlytics or similar libraries, you must initialize them AFTER this crash tool! Installing anyway, but your original handler will not be called.");
            }
            ...
            LogUtil.i(LogUtil.TAG, "Crash tool has been installed.");
        } catch (Throwable t) {
            Log.e(LogUtil.TAG, "An unknown error occurred while installing crash tool, it may not have been properly initialized. Please report this as a bug if needed.", t);
        }
    ...
    private static Class<? extends Activity> obtainActivityByIntentAction(String action) {
        ...
        if (resolveInfos != null && resolveInfos.size() > 0) {
            ...
            try {
                ...
                Log.e(LogUtil.TAG, "Failed when resolving the error activity class via intent filter, stack trace follows!", e);
            }
        }
        return null;
    }
...
//        }
    ...
    private static class MyUncaughtExceptionHandler implements Thread.UncaughtExceptionHandler {
        ...
        public void uncaughtException(Thread thread, final Throwable throwable) {
            Log.e(LogUtil.TAG, "App has crashed, executing UncaughtExceptionHandler", throwable);
            ...
            if (crashActivityClass == null) {
                LogUtil.e(LogUtil.TAG, "Your crash activity not available, must declare in AndroidManifest.xml use intent-filter action: " + INTENT_ACTION_CRASH_ACTIVITY);

//                new Thread() {
//                    @Override
//                    public void run() {
                try {
                    String path = Environment.getExternalStorageDirectory().getAbsolutePath() + "/MyCrashs/";
                    String fileName = TimeUtil.getFormatted("yyyyMMddHHmmss", Calendar.getInstance()) + "_crash.log";
                    ...
                    writer.close();
                    LogUtil.i(LogUtil.TAG, "Save stack trace: " + file.getAbsolutePath());
                } catch (Exception e) {
                    Log.e(LogUtil.TAG, "Save stack trace failed", e);
                }
//                    }
//                }.start();
            } else {
                if (isStackTraceLikelyConflict(throwable, crashActivityClass)) {
                    LogUtil.e(LogUtil.TAG, "Your application class or your crash activity have crashed, the custom activity will not be launched!");
                } else {
            ...
            if (lastActivity != null) {
                LogUtil.i(LogUtil.TAG, "Last activity: " + lastActivity.getClass().getSimpleName());
