...
import com.fxq.apt.annotation.Modules;
...
public class RouterProcessor extends AbstractProcessor {
    ...
    private void generateDefaultRouterInit() {
        ...
        TypeSpec routerInit = TypeSpec.classBuilder("RouterInit")
                ...
                .build();
        try {
            JavaFile.builder("com.fxq.pagerouter.router", routerInit)
                    .build()
                    .writeTo(filer);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private boolean handleRouter(String genClassName, RoundEnvironment roundEnv) {
        ...
        MethodSpec.Builder mapMethod = MethodSpec.methodBuilder("map")
                ...
                .addStatement("com.fxq.pagerouter.router.ExtraTypes extraTypes")
                .addCode("\n");
        ...
        for (Element element : elements) {
            ...
            mapMethod.addStatement("extraTypes = new com.fxq.pagerouter.router.ExtraTypes()");
            ...
            for (String format : router.value()) {
                ...
                if (element.getKind() == ElementKind.CLASS) {
                    mapMethod.addStatement("com.fxq.pagerouter.router.Routers.map($S, $T.class, null, extraTypes)", format, className);
                } else {
                    mapMethod.addStatement("com.fxq.pagerouter.router.Routers.map($S, null, " +
                            "new MethodInvoker() {\n" +
        ...
        TypeSpec routerMapping = TypeSpec.classBuilder(genClassName)
                ...
                .build();
        try {
            JavaFile.builder("com.fxq.pagerouter.router", routerMapping)
                    .build()
                    .writeTo(filer);
        } catch (Throwable e) {
            e.printStackTrace();
